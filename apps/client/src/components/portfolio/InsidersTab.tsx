...
export function InsidersTab({
  ...
  // With no symbol selected, land on the first portfolio holding — mirrors the
  // old /insiders auto-select. Only runs while this tab is mounted (active).
  useEffect(() => {
    const first = watch.data?.items[0]?.symbol;
    if (!symbol && first) {
      onSelectSymbol(first);
    }
  }, [onSelectSymbol, symbol, watch.data?.items]);
    ...
    <>
      ...
      {analysis.data ? (
        <>
          ...
          <Panel variant="inverted" style={styles.hero}>
...
const styles = StyleSheet.create({
  ...
  watchMutedActive: { color: colors.inkOnDarkMuted },
  watchMspr: {
    color: colors.ink,
    fontSize: 23,
    fontWeight: '700',
  ...
  heroTicker: { color: colors.inkOnDarkMuted, fontSize: 9, letterSpacing: 0.6, marginTop: 5 },
  signalPill: { paddingHorizontal: 9, paddingVertical: 7, borderWidth: 1 },
  signalPositive: { borderColor: colors.positiveOnDarkBorder, backgroundColor: colors.positiveOnDarkSoft },
  signalNegative: { borderColor: colors.negativeOnDarkBorder, backgroundColor: colors.negativeOnDarkSoft },
  signalNeutral: { borderColor: colors.hairlineOnDarkStrong, backgroundColor: colors.navyRaised },
  signalText: { fontSize: 8, fontWeight: '800', letterSpacing: 0.6 },
  signalPositiveText: { color: colors.positiveOnDark },
  signalNegativeText: { color: colors.negativeOnDark },
  signalNeutralText: { color: colors.inkOnDarkMuted },
  heroMetrics: {
    marginTop: 22,
    flexDirection: 'row',
    alignItems: 'flex-end',
  ...
  aiPanel: { borderColor: roles.accentSoftBorder },
  aiHeader: {
    minHeight: 66,
    backgroundColor: colors.tealSoft,
  ...
  guideLine: {
    ...
    right: 0,
    height: 1,
    backgroundColor: roles.borderHairline,
  },
  guideTop: { top: 0 },
  guideMiddle: { top: 60, backgroundColor: roles.borderHairlineStrong },
  guideBottom: { top: 120 },
  ...
  positionPanel: { padding: 17, backgroundColor: colors.tealSoft, borderColor: roles.accentSoftBorder },
